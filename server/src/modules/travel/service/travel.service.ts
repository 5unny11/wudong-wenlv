...
export class TravelService {
  ...
  async listScenicSpots(keyword?: string) {
    ...
    return await this.scenicSpotModel.find({ where, order: { id: 'ASC' } });
  }
  ...
  async listRoutes(duration?: string) {
    ...
    return await this.tourRouteModel.find({ where, order: { id: 'ASC' } });
  }
  ...
  async verifyETicket(qrCode: string, merchantId: number) {
    const ticket = await this.eTicketModel.findOne({ where: { qr_code: qrCode, status: 0 } });
